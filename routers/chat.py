...
def Chat(input:Message,rules=Depends(get_rules)):
    ...
    for queries, responses in rules.items():
        ...
        if similarity > 0.8:#If similarity between strings are more than 80%, return the response
            result = dynamic_query_result(queries)#Returns an outcome if the query response is dynamic
            if result:
                return result
